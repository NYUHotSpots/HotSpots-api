...
class EndpointTestCase(TestCase):
    ...
    def test_get_all_spots(self):
        response = self.client.get("/spot/list", headers=self.headers)
        print(response.data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_spot_crud(self):
        ...
        response = self.client.delete(f"/spot/delete/{spot_id}", headers=self.headers)
    ...
    def test_review_crud(self):
        ...
        response = self.client.get(f"/review/read/{spot_id}", headers=self.headers)
    ...
    def test_unauthorized_bad_requests(self):
        response = self.client.post("/spot/create", data=self.spotData)
        self.assertEqual(response.status_code, 401)
        
        bad_header = {"authorization" : "Bearer "}
        response = self.client.post("/spot/create", data=self.spotData, headers=bad_header)
    ...
    def test_bad_deletes(self):
        ...
        '''
        response = self.client.delete(f"/review/delete/1234", headers=self.headers)
        self.assertEqual(response.status_code, 404)
        
        response = self.client.delete(f"/review/delete/{self.bad_id}", headers=self.headers)
        self.assertEqual(response.status_code, 404)
        
        response = self.client.delete(f"/spot/delete/1234", headers=self.headers)
        self.assertEqual(response.status_code, 404)
